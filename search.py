...
def iterativeDeepeningSearch(problem):
    ...
    """
    #Referenced code from the book and online
    x = 1
    while True:
        #init queues
        visited = util.Queue() 
        ...
        border = util.Stack()
        #recursively search
        result = Recursive(problem.getStartState(), problem, x, solution, visited, border)
        x += 1
        if result != 0:
            print (result)
            return solution.list

def Recursive(node, problem, limit, solution, visited, border):
    visited.push(node)
    ...
    if problem.goalTest(node):
        ...
        for action in actions.list:
            ...
            if visited.list.count(child) == 0 and border.list.count(child) == 0:
                result = Recursive(child, problem, limit - 1, solution, visited, border)
                if result == 0:
                    cut = True
